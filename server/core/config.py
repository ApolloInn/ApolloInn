...
# Kiro API actual limit is 128K for all models
...
# ==================================================================================================
# Context Compression (Smart input compression to prevent output truncation)
# ==================================================================================================

# Enable smart context compression.
# When enabled, the gateway will intelligently compress input context when it
# approaches the model's context window limit, preventing Kiro API from
# truncating tool call outputs (e.g., Write with large file content).
#
# Compression strategy (applied in order, stops when target is reached):
#   Level 1: Remove truncation retry loops (zero information loss)
#   Level 2: Compress large tool_results — extract file skeletons (function
#            signatures), compress search results, keep head+tail of long text.
#            Earlier messages are compressed more aggressively than recent ones.
#
# Set to false to disable.
# Default: true
CONTEXT_COMPRESSION: bool = os.getenv("CONTEXT_COMPRESSION", "true").lower() in ("true", "1", "yes")

# ==================================================================================================
# Tool Result Compression (Reduce context size without losing information)
# ==================================================================================================

# Maximum length of tool result content (role=tool messages) in characters.
# Tool results longer than this will be truncated: keep head + tail, replace middle.
# This significantly reduces context size for Kiro API, preventing lazy stops.
# Only affects role=tool messages; user/assistant messages are untouched.
# Set to 0 to disable.
# Default: 15000 chars (~3750 tokens)
TOOL_RESULT_MAX_LENGTH: int = int(os.getenv("TOOL_RESULT_MAX_LENGTH", "15000"))

# How many characters to keep from the beginning of a truncated tool result.
# Default: 10000 chars
TOOL_RESULT_KEEP_HEAD: int = int(os.getenv("TOOL_RESULT_KEEP_HEAD", "10000"))

# How many characters to keep from the end of a truncated tool result.
# Default: 5000 chars
TOOL_RESULT_KEEP_TAIL: int = int(os.getenv("TOOL_RESULT_KEEP_TAIL", "5000"))

# ==================================================================================================
# Anti-Lazy Stop (Prevent model from prematurely stopping)
# ==================================================================================================

# When the model returns finish_reason=stop with very few completion tokens,
# it likely "gave up" on a long task instead of continuing with tool calls.
# By changing finish_reason from "stop" to "length", Cursor will automatically
# send a "continue" message, forcing the model to keep working.
#
# This only applies when:
# 1. finish_reason is "stop" (not tool_calls)
# 2. completion_tokens <= threshold
# 3. The request had tools available (agent mode)
#
# Set to 0 to disable.
# Default: 200 tokens (very short stop replies are suspicious in agent mode)
ANTI_LAZY_STOP_THRESHOLD: int = int(os.getenv("ANTI_LAZY_STOP_THRESHOLD", "100"))
...
# Default: 2500 tokens (balanced: enough for reasoning, leaves ~4K+ for actual output in ~8K budget)
FAKE_REASONING_MAX_TOKENS: int = int(os.getenv("FAKE_REASONING_MAX_TOKENS", "2500"))
