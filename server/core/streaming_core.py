...
from core.parsers import AwsEventStreamParser, parse_bracket_tool_calls, deduplicate_tool_calls
...
from core.thinking_parser import ThinkingParser
...
async def parse_kiro_stream(
    ...
    try:
        ...
        # Tool calls are now yielded incrementally via tool_start/tool_input/tool_complete events
        # from _process_chunk. But we still need to finalize any incomplete tool calls at end of stream.
        # This handles the case where the stream ends without a tool_stop/messageStop event.
        # With interleaved tool calls, there may be multiple active tool calls.
        if parser._active_tool_calls:
            for tid, tc in list(parser._active_tool_calls.items()):
                logger.debug(f"Finalizing incomplete tool call at end of stream: {tc.get('function', {}).get('name', 'unknown')} (id={tid[:20]})")
                parser.current_tool_call = tc
                parser._finalize_tool_call()
                if parser.tool_calls:
                    yield KiroEvent(type="tool_complete", tool_use=parser.tool_calls[-1])
            parser._active_tool_calls.clear()
        elif parser.current_tool_call:
            logger.debug(f"Finalizing incomplete tool call at end of stream: {parser.current_tool_call.get('function', {}).get('name', 'unknown')}")
            parser._finalize_tool_call()
            if parser.tool_calls:
                yield KiroEvent(type="tool_complete", tool_use=parser.tool_calls[-1])
...
async def _process_chunk(
    ...
    for event in events:
        ...
        elif event["type"] in ("tool_start", "tool_input", "tool_complete"):
            # Yield tool events for incremental streaming
            yield KiroEvent(type=event["type"], tool_use=event["data"])
        
        elif event["type"] == "message_stop":
            # Binary parser detected messageStopEvent — yield so streaming layer
            # can emit finish_reason immediately
            yield KiroEvent(type="message_stop", tool_use=event["data"])
        
        elif event["type"] == "metrics":
            # Token usage metrics from metricsEvent
            yield KiroEvent(type="metrics", usage=event["data"])
...
async def collect_stream_to_result(
    ...
    async for event in parse_kiro_stream(response, first_token_timeout, enable_thinking_parser):
        if event.type == "content" and event.content:
            ...
            result.tool_calls.append(event.tool_use)
        elif event.type in ("tool_start", "tool_input"):
            pass  # Incremental events, tool_complete has the final data
        elif event.type in ("message_stop", "metrics"):
            pass  # Handled by streaming layer
