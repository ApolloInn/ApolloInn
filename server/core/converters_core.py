    ...
    if isinstance(content, list):
        ...
        for item in content:
            if isinstance(item, dict):
                # Skip non-text blocks - they're handled separately
                if item.get("type") in ("image", "image_url", "tool_use", "tool_result"):
                    continue
...
def get_chunked_writing_system_addition() -> str:
    """
    Generate system prompt addition for chunked file writing guidance.
    
    The Kiro API has a hard output limit of ~8K completion tokens. With fake reasoning
    consuming ~4K tokens, only ~4K tokens remain for actual output. In tool_call mode,
    this means Write/StrReplace arguments get truncated if the content is too large.
    
    This guidance teaches the model to split large file writes into smaller chunks
    to avoid truncation and the resulting "Invalid arguments" error loop.
    ...
    return (
        "\n\n---\n"
        "# Output Size Limit\n\n"
        "Due to API constraints, each tool call is limited to ~10000 characters of content. "
        "If you exceed this, the output will be silently truncated and the tool call will fail.\n\n"
        "For file creation/editing:\n"
        "1. Write tool: limit content to ~9000 characters per call\n"
        "2. For larger files: Write the first ~9000 chars, then use StrReplace calls "
        "to append remaining content (match the last 1-2 lines, replace with those lines + new content)\n"
        "3. If a Write or StrReplace returns an error, your content was too large — retry with less content"
    )
...
def convert_tool_results_to_kiro_format(tool_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    Returns:
        List of tool results in Kiro format (deduplicated by toolUseId)
    """
    kiro_results = []
    seen_ids = set()
    for tr in tool_results:
        tool_use_id = tr.get("tool_use_id", "")
        # Deduplicate by toolUseId (Claude Code can send duplicate tool_result blocks)
        if tool_use_id and tool_use_id in seen_ids:
            logger.debug(f"Skipping duplicate toolResult: id={tool_use_id}")
            continue
        if tool_use_id:
...
def extract_tool_uses_from_message(
            ...
            if isinstance(tc, dict):
                ...
                if isinstance(arguments, str):
                    try:
                        ...
                        # Malformed arguments (e.g., from a previously truncated tool call)
                        # Pass as-is string wrapped in dict so Kiro API doesn't reject
                        input_data = {"_raw": arguments[:500]} if arguments else {}
                        logger.warning(
                            f"Malformed tool_call arguments for '{func.get('name', '?')}', "
                            f"passing truncated raw ({len(arguments)} chars)"
                        )
                else:
    ...
    # Deduplicate by toolUseId (Claude Code can send duplicate tool_use blocks)
    if tool_uses:
        seen_ids = set()
        deduped = []
        for tu in tool_uses:
            ...
            if tid and tid in seen_ids:
                logger.debug(f"Skipping duplicate toolUse: {tu.get('name', '')} id={tid}")
                continue
            if tid:
                seen_ids.add(tid)
            deduped.append(tu)
...
def build_kiro_payload(
    ...
    # Add chunked writing guidance when tools are present (agent mode)
    # This prevents the model from attempting large Write/StrReplace calls
    # that get truncated by Kiro API's ~8K completion token limit
    if tools:
        chunked_writing_addition = get_chunked_writing_system_addition()
        full_system_prompt = full_system_prompt + chunked_writing_addition if full_system_prompt else chunked_writing_addition.strip()
