...
async function checkAgent(){
  ...
  try{
    ...
    if(licenseActivated){
      ...
      body.innerHTML = `<div style="padding:10px 12px;background:rgba(0,0,0,.03);border-radius:8px;font-size:11px;line-height:1.6"><div style="display:flex;align-items:center;gap:6px;margin-bottom:6px"><span class="material-symbols-rounded" style="color:var(--c-ok);font-size:16px">check_circle</span><span style="color:var(--c-text)">Agent 已连接，正在自动激活...</span></div><div style="color:var(--c-sec)">系统将自动为你分配激活码并完成激活，请稍候。</div></div>`;
      // 自动激活
      try{
        ...
        if(ar.activation_code){
          ...
          if(actRes.ok){ licenseActivated=true; checkAgent(); }
        }
      } catch(e){}
    }
  } catch {
