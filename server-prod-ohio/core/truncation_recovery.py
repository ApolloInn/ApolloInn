...
def generate_truncation_tool_result(
    ...
    """
    content = (
        f"[API Limitation] Your {tool_name} tool call was truncated by the upstream API — "
        f"only {truncation_info.get('size_bytes', 'unknown')} bytes were transmitted before cutoff. "
        f"The file was NOT written successfully.\n\n"
        f"You MUST retry this write immediately. Split the content into smaller chunks:\n"
        f"1. First call: write the first ~80 lines of the file\n"
        f"2. Then use additional calls to append the remaining sections\n\n"
        f"Do NOT skip this file. Do NOT just acknowledge the error. Retry NOW with smaller content."
    )
