...
def normalize_model_name(name: str) -> str:
    ...
    # Strip -thinking suffix before normalization
    # e.g., claude-opus-4-6-thinking → normalize("claude-opus-4-6") → claude-opus-4.6
    # The caller (proxy.py) keeps the original model name for SSE responses
    is_thinking = name_lower.endswith('-thinking')
    if is_thinking:
        name_lower = name_lower[:-len('-thinking')]
    ...
    # If -thinking was stripped, return name_lower (without -thinking)
    # Otherwise return original case for passthrough
    return name_lower if is_thinking else name
