...
# ==================================================================================================
# OpenAI ↔ Anthropic 消息格式适配器
# ==================================================================================================

def _detect_openai_tool_format(messages: List[Dict[str, Any]]) -> bool:
    """检测消息是否使用 OpenAI tool 格式（独立的 role='tool' 消息）。"""
    for m in messages:
        if m.get("role") == "tool":
            return True
    return False
...
def _openai_to_anthropic(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    将 OpenAI 格式的 tool 消息转换为 Anthropic 格式。
    ...
    while i < len(messages):
        ...
        if role == "assistant" and m.get("tool_calls"):
            # Convert assistant with tool_calls to Anthropic format
            content_blocks = []
            ...
            new_m = {"role": "assistant", "content": content_blocks}
            if reasoning:
                new_m["_reasoning_content"] = reasoning
            # Store original format metadata for reverse conversion
            new_m["_openai_tool_calls"] = m["tool_calls"]
            new_m["_openai_content"] = m.get("content", "")
            result.append(new_m)
            i += 1
            ...
            while i < len(messages) and messages[i].get("role") == "tool":
                ...
                tool_result_blocks.append({
                    ...
                    # Preserve original fields for reverse conversion
                    "_tool_name": tm.get("name"),
                })
                i += 1
            result.append({"role": "user", "content": tool_result_blocks, "_synthetic_user": True})

        else:
            # Pass through system, user, plain assistant messages
...
def _anthropic_to_openai(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    将 Anthropic 格式转回 OpenAI 格式。

    只转换带有 _openai_tool_calls 或 _synthetic_user 标记的消息，
    其他消息原样保留。
    """
    result = []
    for m in messages:
        if m.get("role") == "assistant" and "_openai_tool_calls" in m:
            # Reconstruct OpenAI assistant message
            content = m.get("content", [])
            # Extract text from content blocks (may have been modified by compression)
            text_parts = []
            if isinstance(content, list):
                for block in content:
                    if isinstance(block, dict) and block.get("type") == "text":
                            t = block.get("text", ""); text_parts.append(t if isinstance(t, str) else str(t))
            text_content = "\n".join(text_parts) if text_parts else m.get("_openai_content", "")

            new_m = {
                "role": "assistant",
                "content": text_content,
                "tool_calls": m["_openai_tool_calls"],
            }
            reasoning = m.get("_reasoning_content")
            if reasoning:
                new_m["reasoning_content"] = reasoning
            result.append(new_m)

        elif m.get("_synthetic_user"):
            # Convert tool_result blocks back to individual tool messages
            content = m.get("content", [])
            if isinstance(content, list):
                for block in content:
                    if isinstance(block, dict) and block.get("type") == "tool_result":
                        tool_content = block.get("content", "")
                        # If content was wrapped in nested structure by compression, extract text
                        if isinstance(tool_content, list):
                            parts = []
                            for sub in tool_content:
                                if isinstance(sub, dict) and sub.get("type") == "text":
                                       t = sub.get("text", ""); parts.append(t if isinstance(t, str) else str(t))
                        ...
                        if block.get("_tool_name"):
                            new_m["name"] = block["_tool_name"]
                        result.append(new_m)
            else:
                # Fallback: content was replaced with string (shouldn't happen)
                result.append({"role": "user", "content": content})

        else:
            # Clean up any internal markers from pass-through messages
...
def _maybe_convert_back(messages, stats, is_openai):
    """If input was OpenAI format, convert compressed result back."""
    if is_openai:
...
def compress_context(
    ...
    _is_openai_format = _detect_openai_tool_format(messages)
    if _is_openai_format:
        messages = _openai_to_anthropic(messages)
        original_tokens = estimate_request_tokens(messages, tools)
        logger.info(f"[Compression] OpenAI format detected, converted {len(messages)} messages to Anthropic format")
    ...
    if is_subagent:
        ...
        return _maybe_convert_back(current, stats, _is_openai_format)
