...
from core.models_openai import ChatMessage, ChatCompletionRequest, Tool
...
def _extract_tool_calls_from_openai(msg: ChatMessage) -> List[Dict[str, Any]]:
    ...
    1. OpenAI format: msg.tool_calls = [{"id": "...", "function": {"name": "...", "arguments": "..."}}]
    2. Cursor/Anthropic format: msg.content = [{"type": "tool_use", "id": "...", "name": "...", "input": {...}}]
    ...
    # Format 1: OpenAI tool_calls field
    if msg.tool_calls:
    ...
    # Format 2: Cursor/Anthropic content blocks with type="tool_use"
    ...
    for msg in non_system_messages:
        if msg.role == "tool":
            ...
            if msg.role == "assistant":
                ...
                # DEBUG: log content structure for large user messages (likely contains images)
                if isinstance(msg.content, list):
                    block_types = [b.get("type") if isinstance(b, dict) else type(b).__name__ for b in msg.content]
                    if "image_url" in block_types or "image" in block_types:
                        logger.info(f"[IMAGE DEBUG] user msg has image blocks: types={block_types}, extracted={len(images) if images else 0}")
                elif isinstance(msg.content, str) and len(msg.content) > 50000:
                    logger.info(f"[IMAGE DEBUG] large user msg is str, len={len(msg.content)}, starts_with={msg.content[:100]}")
