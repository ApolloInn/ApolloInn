...
from core.config import MAX_RETRIES, BASE_RETRY_DELAY, FIRST_TOKEN_MAX_RETRIES, STREAMING_READ_TIMEOUT
from core.auth import KiroAuthManager
from core.utils import get_kiro_headers
from core.network_errors import classify_network_error, get_short_error_message, NetworkErrorInfo


# ── 致命 403 检测 ──
# 这些关键词出现在 Kiro API 的 403 响应中时，表示 token 已永久失效，
# 不应重试，应立即禁用该 token。
_FATAL_403_KEYWORDS = [
...
# ── "电脑太多" 检测 ──
# Cursor API 在同一账号 24h 内被太多不同机器使用时返回此错误。
# 不是永久封禁，但需要临时冻结该账号 24h，换其他账号继续。
_TOO_MANY_MACHINES_KEYWORDS = [
    "too many computers",
    "too many machines",
    "too many devices",
    "device limit",
...
# ── 402 月度配额检测 ──
# Kiro API 返回 402 + MONTHLY_REQUEST_COUNT 表示该 token 当月请求次数已用完。
# 不应重试同一 token，应立即轮换到下一个 token。
_QUOTA_402_REASONS = [
...
def is_fatal_403(response_body: str) -> bool:
    """检测 403 响应是否为致命错误（token 永久失效）。"""
    lower = response_body.lower()
    return any(kw in lower for kw in _FATAL_403_KEYWORDS)


def is_too_many_machines(status_code: int, response_body: str) -> bool:
    """检测响应是否为"电脑太多"错误（同账号 24h 内机器数超限）。"""
    if status_code not in (401, 403, 429):
        return False
    lower = response_body.lower()
    return any(kw in lower for kw in _TOO_MANY_MACHINES_KEYWORDS)


def is_quota_402(response_body: str) -> bool:
    """检测 402 响应是否为月度配额耗尽。"""
    lower = response_body.lower()
    return any(kw in lower for kw in _QUOTA_402_REASONS)


def get_fatal_403_reason(response_body: str) -> str:
    """从 403 响应体中提取原因描述。"""
    try:
        data = _json.loads(response_body)
        reason = data.get("reason") or ""
        message = data.get("message") or ""
        return reason if reason else message[:100]
    except Exception:
        return response_body[:100]


def get_402_reason(response_body: str) -> str:
    """从 402 响应体中提取原因描述。"""
    try:
        data = _json.loads(response_body)
        reason = data.get("reason") or ""
    ...
    async def request_with_retry(
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 402 - monthly quota exhausted, don't retry same token
                if response.status_code == 402:
                    ...
                    if is_quota_402(error_body):
                        reason = get_402_reason(error_body)
                        logger.warning(
                            f"Quota 402 detected: {reason} — token monthly limit reached, "
                            f"returning to caller for token rotation"
                        )
                        response._quota_402 = True
                        response._quota_reason = reason
                        return response

                    # 非配额 402，直接返回
                    return response

                # 403 - check if fatal (account suspended/disabled) or just token expired
                if response.status_code == 403:
                    ...
                    # 先检测"电脑太多"（非永久封禁，需临时冻结）
                    if is_too_many_machines(response.status_code, error_body):
                        reason = get_fatal_403_reason(error_body)
                        logger.warning(
                            f"Too-many-machines detected: {reason} — "
                            f"account needs 24h freeze, returning to caller for rotation"
                        )
                        response._too_many_machines = True
                        response._too_many_machines_reason = reason
                        return response

                    if is_fatal_403(error_body):
                        reason = get_fatal_403_reason(error_body)
                        logger.error(
                            f"Fatal 403 detected: {reason} — token is permanently invalid, "
                            f"skipping retry and returning to caller for token rotation/disable"
                        )
                        # 标记在 response 上，让上层 proxy.py 能识别
                        response._fatal_403 = True
                        response._fatal_reason = reason
                        return response

                    # 普通 403（并发冲突、token 过期等），刷新后重试
                ...
                if response.status_code == 429:
                    # 先检测是否"电脑太多"（不是普通 rate limit）
                    try:
                        if stream:
                            body_bytes = await response.aread()
                            error_body_429 = body_bytes.decode("utf-8", errors="replace")
                        else:
                            error_body_429 = response.text
                    except Exception:
                        error_body_429 = ""

                    if is_too_many_machines(response.status_code, error_body_429):
                        reason = get_fatal_403_reason(error_body_429)
                        logger.warning(
                            f"Too-many-machines detected (429): {reason} — "
                            f"account needs 24h freeze, returning to caller for rotation"
                        )
                        response._too_many_machines = True
                        response._too_many_machines_reason = reason
