    ...
    <div class="main-scroll">
      <div class="main-inner">
        ...
        <div class="card" style="border:1px solid rgba(239,68,68,.15)">
          <div class="card-header">
            <span class="card-title" style="color:var(--c-err)">卸载并清理 Cursor</span>
          </div>
          <div class="card-body" id="cleanBody">
            <div style="font-size:12px;color:var(--c-sec);line-height:1.8">
              <div style="margin-bottom:8px;color:var(--c-text);font-size:13px">此操作将完全清除 Cursor 的所有数据：</div>
              <div style="padding-left:4px">
                · 关闭所有 Cursor 进程<br>
                · 删除 Cursor 应用本体<br>
                · 删除全部用户数据、缓存、配置、扩展<br>
                · 清理 Keychain 凭证
              </div>
...
function renderModels(combos){
  ...
  // 分为 Kiro- 开头（Cursor 用）和其他（Cline/ChatBox 用）
...
async function doCleanCursor(){
  ...
  try{
    ...
    result.innerHTML = '<div style="color:var(--c-err);font-size:12px">Agent 连接失败</div>';
  }
  btn.disabled = false;
