...
def reset_cursor_machine_ids(db_path: Path) -> List[str]:
    ...
    try:
        ...
        # promax 写入一个独立的 UUID（与 devDeviceId 不同）
...
def do_switch(account: dict, proxy_config: dict = None) -> dict:
    ...
    # 6.5 配置 OpenAI 代理（API Key + Base URL + 自定义模型）
    ...
    # 6.6 补丁：防止 Cursor 自动关闭 OpenAI Key 转发
...
def do_promax_switch() -> dict:
    """
    智能换号：调 gateway 服务器的 /user/smart-switch 拿 account，然后本地 do_switch。
    """
    cfg = load_config()
...
def do_byok_setup(account: dict = None, proxy_config: dict = None) -> dict:
    """
    一键配置（Pro 账号模式）：
    - 重置机器码（多用户共享同一 Pro 账号时必须）
    - 写入真实 Pro 账号凭证（JWT token + 真实邮箱）
    - 配置代理（API Key + Base URL + 模型列表）
    - 补丁：防止 Cursor 自动关闭 OpenAI Key 转发
    """
    steps = []
    ...
    # 3. 重置机器码（多用户共享同一 Pro 账号，必须每人不同机器码）
    id_steps = reset_cursor_machine_ids(db_path)
    steps.extend(id_steps)

    # 4. 写入真实 Pro 账号凭证
    if account and account.get("accessToken"):
        email = account.get("email", f"apollo-{uuid.uuid4().hex[:8]}@apolloinn.site")
        creds_account = {
            "accessToken": account["accessToken"],
            "refreshToken": account.get("refreshToken", ""),
            "email": email,
        }
        write_msg = write_cursor_creds(db_path, creds_account)
        steps.append(write_msg)
        ok, verify_msg = verify_account_written(db_path, email)
        steps.append(verify_msg)
    else:
        return {"ok": False, "error": "无可用账号，请联系管理员", "steps": steps}

    # 5. 配置代理（API Key + Base URL + 模型）
    if proxy_config and proxy_config.get("apikey"):
        proxy_steps = configure_cursor_proxy(
            db_path,
            apikey=proxy_config.get("apikey", ""),
    ...
    # 6. 补丁：防止 Cursor 自动关闭 OpenAI Key 转发
    patch_steps = patch_cursor_binary()
    steps.extend(patch_steps)
...
def _discover_cursor_install_paths() -> list:
    """从注册表、快捷方式、WMIC 反查所有 Cursor 安装路径（去重）。"""
    found = set()
    try:
        import winreg
        uninstall_roots = [
                ...
                with winreg.OpenKey(hive, root_path) as root_key:
                    ...
                    while True:
                        try:
                            ...
                            i += 1
                            if "cursor" not in sub_name.lower():
                                continue
    ...
    appdata = os.environ.get("APPDATA", str(home / "AppData" / "Roaming"))
    lnk_dirs.append(desktop)
    lnk_dirs.append(Path(appdata) / "Microsoft" / "Windows" / "Start Menu" / "Programs")
    lnk_dirs.append(Path("C:/ProgramData/Microsoft/Windows/Start Menu/Programs"))
    for lnk_dir in lnk_dirs:
        if not lnk_dir.exists():
            continue
        try:
            for lnk_file in lnk_dir.rglob("*.lnk"):
                if "cursor" not in lnk_file.stem.lower():
                    continue
                try:
                    result = subprocess.run(
                        ["powershell", "-NoProfile", "-Command",
                         f"(New-Object -ComObject WScript.Shell).CreateShortcut('{lnk_file}').TargetPath"],
                        capture_output=True, text=True, timeout=5,
                    )
    ...
    try:
        result = subprocess.run(
            ["wmic", "process", "where", "name like '%cursor%'", "get", "ExecutablePath"],
            capture_output=True, text=True, timeout=10,
        )
        for line in result.stdout.splitlines():
            line = line.strip()
            if line and line.lower().endswith(".exe") and "cursor" in line.lower():
                p = Path(line)
                if p.is_file():
                    p = p.parent
                found.add(str(p))
...
def _clean_cursor_shortcuts(steps: list):
    """删除桌面和开始菜单中的 Cursor 快捷方式。"""
    home = Path.home()
    appdata = os.environ.get("APPDATA", str(home / "AppData" / "Roaming"))
    lnk_dirs = [
        home / "Desktop",
    ...
    for lnk_dir in lnk_dirs:
        ...
        try:
            for lnk_file in lnk_dir.rglob("*.lnk"):
                if "cursor" in lnk_file.stem.lower():
                    try:
                        lnk_file.unlink()
                        removed += 1
                    except Exception:
            ...
            pass
    if removed > 0:
        steps.append(f"已删除 {removed} 个快捷方式")


def _clean_cursor_credentials(steps: list):
    """清理 Windows Credential Manager 中的 Cursor 凭证。"""
    try:
        result = subprocess.run(
        ...
        for line in result.stdout.splitlines():
            line = line.strip()
            if line.startswith("Target:") or line.startswith("目标:"):
                target = line.split(":", 1)[1].strip()
                if "cursor" in target.lower():
                    try:
                        subprocess.run(["cmdkey", "/delete:" + target],
                                       capture_output=True, timeout=5)
                        steps.append(f"已删除凭证: {target}")
                    except Exception:
                        pass
    except Exception:
        pass
...
def do_clean_cursor() -> dict:
    """
    完全清理 Cursor（仅 Windows）：
    关闭进程 → 卸载程序 → 删除所有数据/缓存/配置 → 清理注册表
    """
    if _system != "Windows":
        return {"ok": False, "error": "此功能仅支持 Windows 系统"}

    steps = []

    # 1. 关闭所有 Cursor 进程
    for proc_name in ["Cursor.exe", "Cursor Helper.exe", "Cursor Helper (GPU).exe",
                       "Cursor Helper (Renderer).exe", "Cursor Helper (Plugin).exe"]:
        try:
    ...
    # 2. 动态发现所有安装路径
    discovered = _discover_cursor_install_paths()
    home = Path.home()
    local = os.environ.get("LOCALAPPDATA", str(home / "AppData" / "Local"))
    appdata = os.environ.get("APPDATA", str(home / "AppData" / "Roaming"))
    ...
    all_install_dirs = set()
    for d in discovered:
        all_install_dirs.add(d)
    for d in [
        Path(local) / "Programs" / "cursor",
        Path(local) / "Programs" / "Cursor",
        Path(programfiles) / "Cursor",
        Path(programfiles_x86) / "Cursor",
    ]:
        all_install_dirs.add(str(d))

    # 3. 尝试卸载
    for install_dir in list(all_install_dirs):
        install_path = Path(install_dir)
        update_exe = install_path / "Update.exe"
        if update_exe.exists():
            try:
                subprocess.run([str(update_exe), "--uninstall", "-s"],
                               capture_output=True, timeout=60)
                time.sleep(3)
                steps.append(f"已执行 Squirrel 卸载: {install_path}")
            except Exception as e:
                steps.append(f"Squirrel 卸载失败: {e}")
        for uninstaller_name in ["Uninstall Cursor.exe", "unins000.exe"]:
            uninstaller = install_path / uninstaller_name
            if uninstaller.exists():
                try:
                    subprocess.run([str(uninstaller), "/S", "--force-run"],
                                   capture_output=True, timeout=60)
                    time.sleep(3)
                    steps.append(f"已执行 NSIS 卸载: {uninstaller_name}")
                except Exception as e:
                    steps.append(f"NSIS 卸载失败: {e}")
    ...
    dirs_to_delete = [
        ...
        (home / ".cursor", "项目索引和扩展"),
        (Path(local) / "cursor-updater", "更新缓存(local)"),
        (Path(local) / "Cursor-updater", "更新缓存(local)"),
        (Path(appdata) / "cursor-updater", "更新缓存(roaming)"),
        (Path(appdata) / "Cursor-updater", "更新缓存(roaming)"),
    ]
    for install_dir in all_install_dirs:
        p = Path(install_dir)
        if p.exists():
            dirs_to_delete.append((p, f"安装目录({p})"))
    ...
    # 删除 Temp 中的 Cursor 残留
    try:
        temp_path = Path(temp_dir)
        if temp_path.exists():
            for item in temp_path.iterdir():
                if item.is_dir() and item.name.lower().startswith("cursor"):
                    try:
    ...
    # 5. 清理快捷方式和凭证
    _clean_cursor_shortcuts(steps)
    _clean_cursor_credentials(steps)
                ...
                with winreg.OpenKey(hive, root_path) as root_key:
                    ...
                    while True:
                        try:
                            sub_name = winreg.EnumKey(root_key, i)
                            i += 1
                            if "cursor" in sub_name.lower():
                                cursor_subs.append(sub_name)
                        except OSError:
                            break
                    for sub_name in cursor_subs:
                        try:
                            winreg.DeleteKey(root_key, sub_name)
                            reg_cleaned += 1
                        except Exception:
                            try:
                                subprocess.run(
                                    ["reg", "delete", f"{root_path}\\{sub_name}", "/f"],
                                    capture_output=True, timeout=5)
                                reg_cleaned += 1
                            except Exception:
                                pass
    ...
    static_reg_keys = [
        r"HKCU\Software\Microsoft\Windows\CurrentVersion\App Paths\Cursor.exe",
        r"HKCU\Software\Microsoft\Windows\CurrentVersion\App Paths\cursor.exe",
        r"HKLM\Software\Microsoft\Windows\CurrentVersion\App Paths\Cursor.exe",
        r"HKLM\Software\Microsoft\Windows\CurrentVersion\App Paths\cursor.exe",
    ]
    for key in static_reg_keys:
        try:
    ...
    if reg_cleaned > 0:
        steps.append(f"已清理 {reg_cleaned} 条注册表项")
    else:
        steps.append("注册表无残留")
