...
## 九、ApolloExtractor 凭证提取器构建

提取器是独立的轻量 GUI 工具，用于扫描本机 Kiro / Cursor 凭证并上传到 Apollo Gateway。

### 文件位置

```
client/extractor/
├── apollo_extractor.py    ← 主程序（tkinter GUI）
├── build.sh               ← macOS 构建脚本（PyArmor 加密 + 图标）
├── build.bat              ← Windows 构建脚本（PyArmor 加密 + 图标）
├── icon.icns              ← macOS 图标
└── icon.ico               ← Windows 图标
```
...
产物：`dist/ApolloExtractor.app`

打 DMG（可选）：
```bash
mkdir -p /tmp/edm && cp -R dist/ApolloExtractor.app /tmp/edm/ && ln -s /Applications /tmp/edm/Applications
hdiutil create -volname ApolloExtractor -srcfolder /tmp/edm -ov -format UDZO dist/ApolloExtractor.dmg
rm -rf /tmp/edm
```
...
产物：`dist\ApolloExtractor.exe`

### 环境要求

- Python 3.10+（需要 tkinter，Windows 自带，macOS 可能需要 `brew install python-tk@3.10`）
- pip install pyinstaller
- pip install pyarmor（可选，加密用）

### 上传到 GitHub Releases

```bash
# macOS DMG
gh release upload v1.0.0-extractor dist/ApolloExtractor.dmg --repo ApolloInn/ApolloInn --clobber

# Windows EXE
gh release create v1.0.0-extractor-win --repo ApolloInn/ApolloInn --title "ApolloExtractor Windows" --notes "凭证提取器 Windows 版"
gh release upload v1.0.0-extractor-win dist/ApolloExtractor.exe --repo ApolloInn/ApolloInn --clobber
```

### macOS 首次打开

用户下载后需要解除 Gatekeeper 隔离：
```bash
xattr -cr ApolloExtractor.app
```
或右键 → 打开。
