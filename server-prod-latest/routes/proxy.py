...
# Import Antigravity handler
from routes.antigravity import handle_antigravity_request

# Import Orchids handler
from routes.orchids import handle_orchids_request
...
async def list_models(request: Request):
    ...
    is_enterprise = pool.is_enterprise_token(token_entry) if token_entry else False

    models = []
    seen = set()

    # ── Kiro 模型（按账号类型） ──
    from core.config import KIRO_FREE_MODELS, KIRO_ENTERPRISE_MODELS
    for mid in KIRO_FREE_MODELS:
        models.append({"id": mid, "object": "model", "created": now, "owned_by": "kiro"})
        seen.add(mid)
    if is_enterprise:
        for mid in KIRO_ENTERPRISE_MODELS:
            models.append({"id": mid, "object": "model", "created": now, "owned_by": "kiro"})
            seen.add(mid)

    # ── Antigravity 模型（所有用户可用） ──
    from core.config import ANTIGRAVITY_MODELS
    for mid in ANTIGRAVITY_MODELS:
        if mid not in seen:
            models.append({"id": mid, "object": "model", "created": now, "owned_by": "antigravity"})
            seen.add(mid)

    # ── Orchids 模型（所有用户可用） ──
    from core.config import ORCHIDS_MODELS
    for mid in ORCHIDS_MODELS:
        if mid not in seen:
            models.append({"id": mid, "object": "model", "created": now, "owned_by": "orchids"})
            seen.add(mid)
...
async def chat_completions(request: Request):
    ...
    if normalized_model != request_data.model:
        logger.info(f"Model normalized: {request_data.model} -> {normalized_model}")

    # ══════════════════════════════════════════════════════════════
    # 三级路由：
    #   1. Antigravity 独占（Gemini / GPT）→ 直接走 Antigravity
    #   2. 企业 Kiro 优先（Opus / Sonnet 4.6）→ 企业 Kiro → Antigravity 备选
    #   3. Free Kiro 优先（Sonnet 4.5 / 4 / Haiku）→ Free Kiro → 企业 Kiro → 报错
    # ══════════════════════════════════════════════════════════════

    from core.config import ANTIGRAVITY_ONLY_MODELS, ANTIGRAVITY_MODEL_MAP, ENTERPRISE_KIRO_WITH_AG_FALLBACK
    from core.config import ORCHIDS_ONLY_MODELS, ORCHIDS_MODEL_MAP

    # ── 0. Orchids 独占模型 ──
    if normalized_model in ORCHIDS_ONLY_MODELS or normalized_model in ORCHIDS_MODEL_MAP:
        orch_model = ORCHIDS_MODEL_MAP.get(normalized_model, normalized_model)
        logger.info(f"[{user['name']}] Route: Orchids → {orch_model}")
        raw_body["model"] = normalized_model
        return await handle_orchids_request(
            request, raw_body, user, stream=request_data.stream
        )

    # ── 1. Antigravity 独占（Gemini / GPT） ──
    ag_resolved = ANTIGRAVITY_MODEL_MAP.get(normalized_model, normalized_model)
    if ag_resolved in ANTIGRAVITY_ONLY_MODELS:
        logger.info(f"[{user['name']}] Route: Antigravity-only → {ag_resolved}")
        raw_body["model"] = ag_resolved
        return await handle_antigravity_request(
            request, raw_body, user, stream=request_data.stream
        )

    # ── 2. 企业 Kiro 优先，Antigravity 备选（Opus / Sonnet 4.6） ──
    if normalized_model in ENTERPRISE_KIRO_WITH_AG_FALLBACK:
        kiro_model, ag_model = ENTERPRISE_KIRO_WITH_AG_FALLBACK[normalized_model]
        # 尝试获取企业版 token
        token_entry = await pool.get_user_token_entry(user, model_id=kiro_model)
        if token_entry and pool.is_enterprise_token(token_entry):
            logger.info(f"[{user['name']}] Route: Enterprise Kiro → {kiro_model}")
            request_data.model = kiro_model
            raw_body["model"] = kiro_model
        else:
            # fallback 到 Antigravity
            logger.info(f"[{user['name']}] Route: No enterprise token, fallback Antigravity → {ag_model}")
            raw_body["model"] = ag_model
            return await handle_antigravity_request(
                request, raw_body, user, stream=request_data.stream
            )
    else:
        # ── 3. Free Kiro 优先（Sonnet 4.5 / 4 / Haiku 等） ──
        token_entry = await pool.get_user_token_entry(user, model_id=normalized_model)

    if not token_entry:
        raise HTTPException(status_code=503, detail="No available Kiro tokens")

    # ── 模型权限检查 ──
    is_allowed, error_msg = pool.check_model_permission(normalized_model, token_entry)
    if not is_allowed:
        logger.warning(f"[{user['name']}] Model permission denied: {error_msg}")
        raise HTTPException(status_code=403, detail=error_msg)

    # 记录使用的 Token 类型
    token_type = "个人版" if not pool.is_enterprise_token(token_entry) else "企业版"
    logger.info(
        f"[{user['name']}] model={request_data.model} stream={request_data.stream} "
        f"token={token_entry['id']} ({token_type}) messages={len(request_data.messages)}"
    )
    ...
    for token_attempt in range(max_token_retries):
        if token_attempt > 0:
            # 换一个新的 token（按模型池子选择，避免换到错误账号池）
            new_token_entry = await pool.get_next_token_for_model(
                normalized_model,
                exclude_token_ids={token_entry["id"]},
            )
        ...
        try:
            ...
            if getattr(response, '_too_many_machines', False):
                ...
                logger.error(
                    ...
                    f"auto-disabling token {token_entry['id'][:8]}... to avoid reuse"
                )
                # 这种错误本质上是该凭证在当前环境不可用；直接禁用，避免轮询重复命中
                try:
                    await pool.set_token_status(token_entry["id"], "disabled", reason=f"too_many_machines: {tmm_reason}")
                except Exception as _e:
                    logger.error(f"Failed to disable token {token_entry['id'][:8]} for too_many_machines: {_e}")
                # 换 token 重试
            ...
            if response.status_code == 402 and getattr(response, '_quota_402', False):
                ...
                logger.warning(
                    ...
                    f"auto-disabling and rotating token (attempt {token_attempt + 1}/{max_token_retries})"
                )
                # 自动禁用配额耗尽的 token，避免后续请求继续轮询到它
                try:
                    await pool.set_token_status(token_entry["id"], "disabled", reason=f"quota_exhausted: {quota_reason}")
                except Exception as _e:
                    logger.error(f"Failed to disable token {token_entry['id'][:8]} for quota: {_e}")
                if token_attempt < max_token_retries - 1:
                    await http_client.close()
                    continue
                # 所有 token 都配额耗尽
            ...
            if request_data.stream:
                ...
                return JSONResponse(
                    ...
                    headers={"X-Token-Id": token_entry["id"]},
                )

        except ValueError as e:
            # auth_manager.get_access_token() 可能抛出 ValueError（例如 refresh 失败且 access_token 已过期）
            # 这类凭证在当前环境不可用：直接禁用，避免轮询一直撞到坏号
            reason = str(e)[:200]
            logger.error(f"[{user['name']}] Token {token_entry['id'][:8]} auth error: {reason}, auto-disabling token")
            try:
                await pool.set_token_status(token_entry["id"], "disabled", reason=f"auth_error: {reason}")
            except Exception as _e:
                logger.error(f"Failed to disable token {token_entry['id'][:8]} after auth error: {_e}")
            try:
                await http_client.close()
            except Exception:
                pass
            if token_attempt < max_token_retries - 1:
                continue
            raise HTTPException(status_code=503, detail="All available tokens failed to refresh/authenticate")
