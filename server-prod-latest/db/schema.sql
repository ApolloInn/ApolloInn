...
-- ── Antigravity (Google Gemini Code Assist) 凭证池 ──
CREATE TABLE IF NOT EXISTS antigravity_tokens (
  ...
  status         TEXT DEFAULT 'active',       -- active / disabled / cooldown
...
CREATE INDEX IF NOT EXISTS idx_ag_tokens_status ON antigravity_tokens(status);
CREATE INDEX IF NOT EXISTS idx_ag_tokens_email ON antigravity_tokens(email);

-- ── Orchids (Orchids AI Coding Agent) 凭证池 ──
CREATE TABLE IF NOT EXISTS orchids_tokens (
  id             TEXT PRIMARY KEY DEFAULT gen_random_uuid()::TEXT,
  email          TEXT DEFAULT '',
  client_cookie  TEXT DEFAULT '',
...
CREATE INDEX IF NOT EXISTS idx_orchids_tokens_status ON orchids_tokens(status);
CREATE INDEX IF NOT EXISTS idx_orchids_tokens_email ON orchids_tokens(email);
