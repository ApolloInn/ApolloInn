...
class TokenPool:
    # 每 token 每秒最大请求数
    TOKEN_RATE_LIMIT = 2
    AG_TOKEN_RATE_LIMIT = 2

    def __init__(self, database_url: str):
        self._dsn = database_url
        self._pool = None
        self._rr_index = 0
        self._model_rr_index = 0  # 按模型选 token 时的轮询下标
        ...
        # Token 限速器：token_id -> [timestamp1, timestamp2, ...]
        self._token_requests: Dict[str, list] = {}
    ...
    def _row_to_token(self, r):
        return {
            ...
            "email_password": r.get("email_password", "") or "",
            "lastUsed": _to_bj(r["last_used"]),
            "useCount": r["use_count"],
            "supportsOpus": r.get("supports_opus", False),
        }
    ...
    async def get_next_token(self, max_wait: float = 10.0):
        """获取下一个可用 token（带限速）。
        
        每个 token 每秒最多 TOKEN_RATE_LIMIT 个请求。
        如果所有 token 都达到限速，等待直到有可用的。
        max_wait: 最大等待时间（秒），超时返回 None
        """
        async with self._pool.acquire() as conn:
        ...
        while True:
            ...
            if now - start_time > max_wait:
                logger.warning(f"Rate limit: all {n} tokens busy, waited {max_wait}s, giving up")
                return None
            
            async with self._rate_lock:
                # 清理过期的请求记录（超过1秒的）
                for tid in list(self._token_requests.keys()):
                    self._token_requests[tid] = [t for t in self._token_requests[tid] if now - t < 1.0]
                
                # 找一个未达到限速的 token（从上次位置开始轮询）
                for i in range(n):
                    idx = (self._rr_index + i) % n
                    token = rows[idx]
                    tid = token['id']
                    
                    requests = self._token_requests.get(tid, [])
                    if len(requests) < self.TOKEN_RATE_LIMIT:
                        # 记录这次请求
                        if tid not in self._token_requests:
                            self._token_requests[tid] = []
                        self._token_requests[tid].append(now)
                        self._rr_index = (idx + 1) % n
                        return self._row_to_token(token)
            
            # 所有 token 都达到限速，等待一小段时间再试
            await asyncio.sleep(0.1)

    async def get_next_token_for_model(
        self,
        model_id: str,
        exclude_token_ids: set[str] | None = None,
    ):
        """
        按模型类型选择下一个可用 token（同 get_user_token_entry/_get_token_for_model 的分类规则），
        并可排除当前 token，避免重试时换到同一个或换到错误池子。
        """
        ...
        # 分类（同 _get_token_for_model）
        is_enterprise_only = model_id in self.ENTERPRISE_ONLY_MODELS
        is_personal_only = model_id in self.PERSONAL_ONLY_MODELS

        personal_tokens = []
        enterprise_tokens = []
        for row in rows:
            t = self._row_to_token(row)
            if t["id"] in exclude_token_ids:
                continue
            if self.is_enterprise_token(t):
                enterprise_tokens.append(t)
            else:
                personal_tokens.append(t)

        if is_enterprise_only:
            candidates = enterprise_tokens
        elif is_personal_only:
            candidates = personal_tokens
        else:
            candidates = personal_tokens + enterprise_tokens

        if not candidates:
            return None

        return await self._select_token_with_rate_limit(candidates)
    ...
    def _row_to_user(self, r, apikeys=None):
        u = {
            ...
            "switch_remaining": r.get("switch_remaining", 0) or 0,
            "claim_remaining": r.get("claim_remaining", 0) or 0,
    ...
    async def decrement_switch_remaining(self, user_id: str) -> int:
        """换号后剩余次数 -1，返回更新后的值。"""
        async with self._pool.acquire() as conn:
            r = await conn.fetchval(
                "UPDATE users SET switch_remaining = GREATEST(COALESCE(switch_remaining, 0) - 1, 0) WHERE id = $1 RETURNING switch_remaining",
                user_id,
            )
            self._auth_cache.clear()
            return r or 0

    async def adjust_switch_remaining(self, user_id: str, delta: int) -> int:
        """管理员调整用户剩余换号次数（正数加，负数减），返回调整后的值。"""
        async with self._pool.acquire() as conn:
            r = await conn.fetchval(
                "UPDATE users SET switch_remaining = GREATEST(COALESCE(switch_remaining, 0) + $1, 0) WHERE id = $2 RETURNING switch_remaining",
                delta, user_id,
            )
            self._auth_cache.clear()
    ...
    async def get_user_token_entry(self, user, model_id: str = None):
        """
        获取用户应该使用的凭证。
        ...
        # 如果指定了模型，根据模型类型智能选择 Token
        if model_id:
            return await self._get_token_for_model(model_id)
        
        # 没有指定模型，使用默认轮询
        return await self.get_next_token()

    async def _get_token_for_model(self, model_id: str):
        """
        根据模型类型智能选择 Token。
        ...
        for row in rows:
            ...
            if self.is_enterprise_token(token):
                enterprise_tokens.append(token)
            else:
                personal_tokens.append(token)
        
        # 根据模型类型选择 Token 池
        if is_enterprise_only:
            # 企业版专属模型：只从企业版 Token 中选择
            candidate_tokens = enterprise_tokens
            logger.debug(f"Model {model_id} is enterprise-only, selecting from {len(candidate_tokens)} enterprise tokens")
        elif is_personal_only:
            # 个人版专属模型：只从个人版 Token 中选择
            candidate_tokens = personal_tokens
            logger.debug(f"Model {model_id} is personal-only, selecting from {len(candidate_tokens)} personal tokens")
        else:
            # 共同模型：优先个人版，无可用时用企业版
            candidate_tokens = personal_tokens + enterprise_tokens
            logger.debug(f"Model {model_id} is common, prioritizing personal tokens ({len(personal_tokens)} personal, {len(enterprise_tokens)} enterprise)")
        
        if not candidate_tokens:
            logger.warning(f"No suitable tokens available for model {model_id}")
            return None
        
        # 从候选 Token 中选择（带限速）
        return await self._select_token_with_rate_limit(candidate_tokens)

    async def _select_token_with_rate_limit(self, candidate_tokens: list):
        """
        从候选 Token 列表中选择一个可用的（带限速 + 轮询）。
        从 _model_rr_index 位置开始轮询，避免总是用同一个 token。
        """
        if not candidate_tokens:
            return None
        ...
        async with self._rate_lock:
            ...
            # 从上次位置开始轮询，找一个未达到限速的 token
            for i in range(n):
                idx = (self._model_rr_index + i) % n
                ...
                if len(requests) < self.TOKEN_RATE_LIMIT:
                    if tid not in self._token_requests:
                        self._token_requests[tid] = []
                    self._token_requests[tid].append(now)
                    self._model_rr_index = (idx + 1) % n
                    return token
        
        # 所有候选 Token 都达到限速，返回当前轮询位（会等待）
        logger.warning(f"All {n} candidate tokens are rate-limited")
        return candidate_tokens[self._model_rr_index % n] if candidate_tokens else None
    ...
    def _row_to_cursor_token(self, r):
        ...
        return {
            ...
            "lastUsed": _to_bj(r["last_used"]),
            "useCount": r["use_count"],
            "frozen_until": _to_bj(r.get("frozen_until")),
        }
    ...
    async def add_cursor_token(self, data: Dict) -> Dict:
        ...
        owner_type = data.get("owner_type", "admin")
                ...
                async with self._pool.acquire() as conn:
                    await conn.execute(
                        """UPDATE cursor_tokens SET access_token=$1, refresh_token=$2,
                           ...
                           machine_ids = $6::jsonb, owner_type=$8, owner_id=$9, email_password=$10
                           WHERE id=$7""",
                        data.get("accessToken", ""), data.get("refreshToken", ""),
                        password, data.get("note", ""), now, machine_ids, tid,
                        owner_type, owner_id, email_password,
                    )
                logger.info(f"Cursor token updated (same email): id={tid} email={email} owner={owner_type}:{owner_id}")
        ...
        async with self._pool.acquire() as conn:
            await conn.execute(
                """INSERT INTO cursor_tokens (id, email, password, access_token, refresh_token, note, status, added_at, use_count, machine_ids, last_refreshed_at, owner_type, owner_id, email_password)
                   VALUES ($1,$2,$3,$4,$5,$6,'active',$7,0,$8::jsonb,$7,$9,$10,$11)""",
                tid, email, password, data.get("accessToken", ""),
                data.get("refreshToken", ""), data.get("note", ""), now, machine_ids,
                owner_type, owner_id, email_password,
            )
        logger.info(f"Cursor token added: id={tid} email={email} owner={owner_type}:{owner_id}")
        return {"id": tid, "email": email, "status": "active",
    ...
    async def pick_cursor_account_for_switch(self, user_name: str, current_email: str = ""):
        """
        智能换号：排除当前账号，选当前实际使用人数最少的账号。
        使用人数 = users 表中 cursor_email 匹配该账号且 status='active' 的用户数。
        只选有有效凭证（refresh_token 非空 或 access_token 为 JWT 格式）的账号。
        如果只有一个账号，也返回它（刷新凭证用）。
        确保返回的账号有 machine_ids，没有则自动生成并存入 DB。
        """
        cred_filter = "(ct.refresh_token != '' OR ct.access_token LIKE 'eyJ%')"
        frozen_filter = "(ct.frozen_until IS NULL OR ct.frozen_until <= NOW())"
        # 按实际使用人数排序，人数相同则选 use_count 最少的
        query_base = f"""
            SELECT ct.*, COALESCE(uc.cnt, 0) AS active_users
            FROM cursor_tokens ct
            LEFT JOIN (
                SELECT cursor_email, COUNT(*) AS cnt
                FROM users WHERE status = 'active' AND cursor_email != ''
                GROUP BY cursor_email
            ) uc ON uc.cursor_email = ct.email
            WHERE ct.status = 'active' AND {cred_filter} AND {frozen_filter}
        """
        MAX_USERS_PER_ACCOUNT = 4
        # 过滤掉已达人数上限的账号
        query_base += f" AND COALESCE(uc.cnt, 0) < {MAX_USERS_PER_ACCOUNT}"
        order = "ORDER BY active_users ASC, ct.use_count ASC, ct.last_used ASC NULLS FIRST LIMIT 1"
        async with self._pool.acquire() as conn:
            # 优先选不同于当前邮箱的
            if current_email:
                r = await conn.fetchrow(
                    f"{query_base} AND ct.email != $1 {order}",
                    current_email,
                )
                if r:
    ...
    async def _ensure_machine_ids(self, conn, row):
        ...
        if isinstance(mid, str):
            try:
                ...
                mid = {}
        if mid and (mid.get("telemetry.machineId") or mid.get("machineId")):
            return row
    ...
    async def freeze_cursor_account(self, email: str, hours: int = 24, reason: str = "") -> bool:
        """
        冻结 Cursor 账号指定时长（默认 24h）。
        用于"电脑太多"等临时限制，到期自动解冻（pick 时检查 frozen_until）。
        """
        frozen_until = datetime.now(timezone.utc) + timedelta(hours=hours)
        async with self._pool.acquire() as conn:
            res = await conn.execute(
                "UPDATE cursor_tokens SET frozen_until = $1 WHERE email = $2",
                frozen_until, email,
            )
        if res == "UPDATE 1":
            logger.warning(
                f"Cursor account {email} frozen until {_to_bj(frozen_until)} "
                f"({hours}h) reason: {reason}"
            )
            return True
        return False
    ...
    async def write_claim_log(self, user_id: str, user_name: str, email: str,
                              action: str = "claim", source: str = "admin", agent_id: str = ""):
        async with self._pool.acquire() as conn:
            await conn.execute(
                """INSERT INTO cursor_claim_logs (user_id, user_name, email, action, source, agent_id)
                   VALUES ($1, $2, $3, $4, $5, $6)""",
                user_id, user_name, email, action, source, agent_id,
            )

    async def list_claim_logs(self, user_id: str = "", email: str = "", agent_id: str = "", limit: int = 200):
        conditions, params = [], []
        idx = 1
        if user_id:
            conditions.append(f"user_id = ${idx}"); params.append(user_id); idx += 1
        if email:
            conditions.append(f"email = ${idx}"); params.append(email); idx += 1
        if agent_id:
            conditions.append(f"agent_id = ${idx}"); params.append(agent_id); idx += 1
        where = (" WHERE " + " AND ".join(conditions)) if conditions else ""
        params.append(limit); idx_limit = idx
        async with self._pool.acquire() as conn:
            rows = await conn.fetch(
                f"SELECT * FROM cursor_claim_logs{where} ORDER BY created_at DESC LIMIT ${idx_limit}",
                *params,
            )
        return [
            {"id": r["id"], "user_id": r["user_id"], "user_name": r["user_name"],
             "email": r["email"], "action": r["action"], "source": r["source"],
             "agent_id": r["agent_id"], "created_at": str(r["created_at"])}
            for r in rows
        ]

    async def revoke_cursor_account(self, user_id: str, operator: str = "admin", agent_id: str = ""):
        """回收用户的 Cursor 账号：清空 cursor_email，写日志。"""
        async with self._pool.acquire() as conn:
            row = await conn.fetchrow("SELECT id, name, cursor_email FROM users WHERE id = $1", user_id)
            if not row or not row["cursor_email"]:
                return {"ok": False, "error": "用户无已领取账号"}
            email = row["cursor_email"]
            user_name = row["name"]
            await conn.execute("UPDATE users SET cursor_email = '' WHERE id = $1", user_id)
            # 释放 cursor_tokens 的 assigned_user
            await conn.execute(
                "UPDATE cursor_tokens SET assigned_user = '' WHERE email = $1 AND assigned_user = $2",
                email, user_name,
            )
        await self.write_claim_log(user_id, user_name, email, action="revoke",
                                   source="agent" if agent_id else "admin", agent_id=agent_id)
        logger.info(f"revoke: {user_name} cursor account {email} revoked by {operator}")
        return {"ok": True, "user_id": user_id, "user_name": user_name, "email": email}
    ...
    @staticmethod
    def is_enterprise_token(token_entry: Dict) -> bool:
        """
        判断 token 是否为企业版。
        
        判断依据：
        - provider 字段不是 'BuilderId'（个人版标识）
        - provider 为 'Enterprise' 或其他非空值
        
        Args:
            token_entry: Token 数据字典
            
        Returns:
            True 表示企业版，False 表示个人版
        """
        provider = token_entry.get("provider", "")
        return provider != "" and provider != "BuilderId"

    @classmethod
    def check_model_permission(cls, model_id: str, token_entry: Dict) -> tuple[bool, Optional[str]]:
        """
        检查 token 是否有权限使用指定模型。
        
        权限规则：
        1. 企业版专属模型（opus 4.5/4.6, sonnet 4.6）只能企业版 token 使用
        2. 个人版专属模型（deepseek, minimax, qwen）只能个人版 token 使用
        3. 共同模型（auto, sonnet 4.5/4, haiku 4.5）两种版本都可以使用
        
        Args:
            model_id: 模型 ID（已解析后的，如 claude-opus-4.6）
            token_entry: Token 数据字典
            
        Returns:
            (is_allowed, error_message)
            - is_allowed: True 表示允许，False 表示拒绝
            - error_message: 拒绝时的错误信息，允许时为 None
        """
        is_enterprise = cls.is_enterprise_token(token_entry)
        provider_type = "企业版" if is_enterprise else "个人版 (BuilderId)"
        
        # 检查企业版专属模型
        if model_id in cls.ENTERPRISE_ONLY_MODELS:
            if not is_enterprise:
                return False, f"模型 '{model_id}' 仅企业版 Token 可用，当前使用的是{provider_type} Token"
            return True, None
        
        # 检查个人版专属模型
        if model_id in cls.PERSONAL_ONLY_MODELS:
            if is_enterprise:
                return False, f"模型 '{model_id}' 仅个人版 Token 可用，当前使用的是{provider_type} Token"
            return True, None
        
        # 共同模型，都可以使用
        return True, None

    async def get_available_models_for_token(self, token_entry: Dict) -> list[str]:
        """
        获取指定 token 可用的模型列表。
        ...
        Returns:
            模型 ID 列表
        """
        is_enterprise = self.is_enterprise_token(token_entry)
        ...
        if is_enterprise:
            # 企业版：共同模型 + 企业版专属模型
            return common_models + list(self.ENTERPRISE_ONLY_MODELS)
        else:
            # 个人版：共同模型 + 个人版专属模型
            return common_models + list(self.PERSONAL_ONLY_MODELS)

    # ══════════════════════════════════════════════════════════════════════
    # Antigravity (Google Gemini Code Assist) Token Pool
    # ══════════════════════════════════════════════════════════════════════

    _ag_rr_index = 0  # Antigravity 轮询下标
    _ag_failure_counts: Dict[str, int] = {}  # token_id → 连续失败次数（用于指数退避）
    _ag_in_flight: Dict[str, int] = {}  # token_id → 当前正在进行的请求数
    AG_MAX_CONCURRENCY = 1  # 每个 token 最大并发数

    def _row_to_ag_token(self, r) -> Dict:
        return {
            "id": r["id"],
            "email": r["email"],
            ...
            "expires_at": _to_bj(r["expires_at"]) if r["expires_at"] else None,
            "status": r["status"],
            ...
            "cooldown_until": _to_bj(r["cooldown_until"]) if r.get("cooldown_until") and r["cooldown_until"] > datetime.now(timezone.utc) else None,
        }

    async def add_ag_token(self, data: Dict) -> Dict:
        """添加 Antigravity 凭证。"""
        email = data.get("email", "")
        now = datetime.now(timezone.utc)
        ...
        if email:
            async with self._pool.acquire() as conn:
                existing = await conn.fetchrow(
                    "SELECT id FROM antigravity_tokens WHERE email = $1", email
                )
            if existing:
                tid = existing["id"]
                async with self._pool.acquire() as conn:
                    await conn.execute(
                        """UPDATE antigravity_tokens SET
                           access_token=$1, refresh_token=$2, project_id=$3,
                           expires_at=$4, status='active', disabled_reason='',
                           error_count=0, cooldown_until=NULL, oauth_client=$6
                           WHERE id=$5""",
                        data.get("access_token", ""), data.get("refresh_token", ""),
                        data.get("project_id", ""),
                        datetime.fromtimestamp(data["expires_at"], tz=timezone.utc) if data.get("expires_at") else None,
                        tid,
                        data.get("oauth_client", "antigravity"),
                    )
                logger.info(f"AG token updated: id={tid} email={email}")
                return {"id": tid, "email": email, "status": "active", "updated": True}

        tid = secrets.token_hex(8)
        oauth_client = data.get("oauth_client", "antigravity")
        async with self._pool.acquire() as conn:
            await conn.execute(
                """INSERT INTO antigravity_tokens
                   (id, email, access_token, refresh_token, project_id, expires_at, status, added_at, oauth_client)
                   VALUES ($1,$2,$3,$4,$5,$6,'active',$7,$8)""",
                tid, email, data.get("access_token", ""), data.get("refresh_token", ""),
                data.get("project_id", ""),
                datetime.fromtimestamp(data["expires_at"], tz=timezone.utc) if data.get("expires_at") else None,
                now, oauth_client,
            )
        logger.info(f"AG token added: id={tid} email={email} client={oauth_client}")
        return {"id": tid, "email": email, "status": "active"}

    async def list_ag_tokens(self):
        """列出所有 Antigravity 凭证（脱敏）。"""
        async with self._pool.acquire() as conn:
            rows = await conn.fetch("SELECT * FROM antigravity_tokens ORDER BY added_at")
        result = []
        for r in rows:
            t = self._row_to_ag_token(r)
            if t["access_token"]:
                t["access_token"] = t["access_token"][:16] + "..."
            if t["refresh_token"]:
                t["refresh_token"] = t["refresh_token"][:16] + "..."
    ...
    async def remove_ag_token(self, token_id: str) -> bool:
        async with self._pool.acquire() as conn:
            res = await conn.execute("DELETE FROM antigravity_tokens WHERE id = $1", token_id)
        return res == "DELETE 1"

    async def get_ag_token_full(self, token_id: str) -> Optional[Dict]:
        async with self._pool.acquire() as conn:
            r = await conn.fetchrow("SELECT * FROM antigravity_tokens WHERE id = $1", token_id)
        if not r:
            return None
        return self._row_to_ag_token(r)

    async def get_next_ag_token(self) -> Optional[Dict]:
        """轮询获取下一个可用的 Antigravity 凭证（带限速）。"""
        import asyncio as _asyncio

        now_utc = datetime.now(timezone.utc)
        async with self._pool.acquire() as conn:
            rows = await conn.fetch(
                """SELECT * FROM antigravity_tokens
                   WHERE status = 'active'
        ...
        for wait_attempt in range(4):
            ...
            async with self._rate_lock:
                ...
                # 轮询找一个未达限速的 token
                for i in range(n):
                    idx = (self._ag_rr_index + i) % n
                    token = self._row_to_ag_token(rows[idx])
                    tid = token["id"]
                    requests = self._token_requests.get(tid, [])

                    in_flight = self._ag_in_flight.get(tid, 0)
                    if len(requests) < self.AG_TOKEN_RATE_LIMIT and in_flight < self.AG_MAX_CONCURRENCY:
                        if tid not in self._token_requests:
                            self._token_requests[tid] = []
                        self._token_requests[tid].append(now)
                        self._ag_rr_index = (idx + 1) % n
                        return token

            if wait_attempt == 0:
                logger.warning(f"All {n} AG tokens are rate-limited, waiting for slot...")

        # 等待后仍然全满，返回当前轮询位（避免完全阻塞）
        logger.warning(f"All {n} AG tokens still rate-limited after wait, dispatching anyway")
        async with self._rate_lock:
            idx = self._ag_rr_index % n
            self._ag_rr_index = (idx + 1) % n
            return self._row_to_ag_token(rows[idx])

    def ag_token_acquire(self, token_id: str):
        """标记 token 开始服务一个请求。"""
        self._ag_in_flight[token_id] = self._ag_in_flight.get(token_id, 0) + 1

    def ag_token_release(self, token_id: str):
        """标记 token 结束一个请求。"""
        cur = self._ag_in_flight.get(token_id, 0)
        if cur > 0:
            self._ag_in_flight[token_id] = cur - 1

    async def mark_ag_token_used(self, token_id: str):
        """标记 Antigravity 凭证已使用。"""
        now = datetime.now(timezone.utc)
        async with self._pool.acquire() as conn:
            await conn.execute(
                "UPDATE antigravity_tokens SET last_used=$1, use_count=use_count+1 WHERE id=$2",
                now, token_id,
            )

    async def update_ag_token_credentials(self, token_id: str, access_token: str,
                                           refresh_token: str = None, expires_at: float = None):
        """刷新后更新 Antigravity 凭证。"""
        async with self._pool.acquire() as conn:
            if refresh_token and expires_at:
                await conn.execute(
                    """UPDATE antigravity_tokens SET access_token=$1, refresh_token=$2,
                       expires_at=$3, error_count=0 WHERE id=$4""",
                    access_token, refresh_token,
                    datetime.fromtimestamp(expires_at, tz=timezone.utc), token_id,
                )
            elif expires_at:
                await conn.execute(
                    "UPDATE antigravity_tokens SET access_token=$1, expires_at=$2, error_count=0 WHERE id=$3",
                    access_token, datetime.fromtimestamp(expires_at, tz=timezone.utc), token_id,
                )
            else:
                await conn.execute(
                    "UPDATE antigravity_tokens SET access_token=$1, error_count=0 WHERE id=$2",
                    access_token, token_id,
                )

    async def update_ag_project_id(self, token_id: str, project_id: str):
        """更新 Antigravity 凭证的 project_id。"""
        async with self._pool.acquire() as conn:
            await conn.execute(
                "UPDATE antigravity_tokens SET project_id=$1 WHERE id=$2",
                project_id, token_id,
            )

    async def disable_ag_token(self, token_id: str, reason: str = ""):
        """禁用 Antigravity 凭证。"""
        async with self._pool.acquire() as conn:
            await conn.execute(
                "UPDATE antigravity_tokens SET status='disabled', disabled_reason=$1 WHERE id=$2",
                reason, token_id,
            )
        logger.warning(f"AG token disabled: {token_id} reason={reason}")

    async def cooldown_ag_token(self, token_id: str, seconds: int = 120):
        """将 Antigravity 凭证置入冷却期。"""
        until = datetime.now(timezone.utc) + timedelta(seconds=seconds)
        async with self._pool.acquire() as conn:
            await conn.execute(
                "UPDATE antigravity_tokens SET cooldown_until=$1, error_count=error_count+1 WHERE id=$2",
                until, token_id,
            )
        logger.info(f"AG token cooldown: {token_id} for {seconds}s")

    # ── 429 错误类型与退避阶梯 ──
    AG_QUOTA_BACKOFF_STEPS = [60, 300, 1800, 7200]

    async def cooldown_ag_token_smart(self, token_id: str, error_body: str = "",
                                       retry_after_header: str = ""):
        """根据 Google 429 响应智能决定冷却时间。"""
        import re as _re

        reason, parsed_seconds = self._parse_ag_429(error_body, retry_after_header)

        if parsed_seconds is not None:
            cooldown_sec = max(parsed_seconds, 2)
            logger.info(f"AG [{token_id[:8]}] {reason}: parsed wait={cooldown_sec}s")
        elif reason == "RATE_LIMIT_EXCEEDED":
            cooldown_sec = 5
        elif reason == "MODEL_CAPACITY_EXHAUSTED":
            failure_count = self._ag_failure_counts.get(token_id, 0)
            cooldown_sec = min(5 + failure_count * 5, 15)
        elif reason == "QUOTA_EXHAUSTED":
            failure_count = self._ag_failure_counts.get(token_id, 0)
            idx = min(failure_count, len(self.AG_QUOTA_BACKOFF_STEPS) - 1)
            cooldown_sec = self.AG_QUOTA_BACKOFF_STEPS[idx]
        else:
            cooldown_sec = 10

        self._ag_failure_counts[token_id] = self._ag_failure_counts.get(token_id, 0) + 1
        logger.warning(f"AG [{token_id[:8]}] 429 type={reason}, cooldown={cooldown_sec}s "
                       f"(failures={self._ag_failure_counts[token_id]})")
        await self.cooldown_ag_token(token_id, seconds=cooldown_sec)

    def mark_ag_token_success(self, token_id: str):
        """请求成功后重置失败计数。"""
        if self._ag_failure_counts.pop(token_id, None):
            logger.debug(f"AG [{token_id[:8]}] success, reset failure count")

    @staticmethod
    def _parse_ag_429(body: str, retry_after_header: str = ""):
        """解析 Google 429 响应，返回 (reason, parsed_seconds)。"""
        import re as _re
        ...
        if retry_after_header:
            try:
                parsed_seconds = int(retry_after_header)
            except ValueError:
                pass
        ...
        if data:
            ...
            if details and isinstance(details, list):
                ...
                r = detail.get("reason", "")
                if r in ("QUOTA_EXHAUSTED", "RATE_LIMIT_EXCEEDED", "MODEL_CAPACITY_EXHAUSTED"):
                    reason = r

                # 解析 quotaResetDelay（如 "2h1m1s"、"42s"、"500ms"）
                metadata = detail.get("metadata", {})
                delay_str = metadata.get("quotaResetDelay", "")
                if delay_str and parsed_seconds is None:
                    parsed_seconds = TokenPool._parse_duration(delay_str)
            ...
            if reason == "UNKNOWN":
                ...
                elif "exhausted" in msg or "quota" in msg:
                    reason = "QUOTA_EXHAUSTED"

        # 3. 纯文本匹配
        if reason == "UNKNOWN" and body:
            body_lower = body.lower()
            if "per minute" in body_lower or "rate limit" in body_lower:
                reason = "RATE_LIMIT_EXCEEDED"
            elif "exhausted" in body_lower or "quota" in body_lower:
                reason = "QUOTA_EXHAUSTED"

        return reason, parsed_seconds

    @staticmethod
    def _parse_duration(s: str):
        """解析 '2h1m42s'、'500ms' 等格式为秒数。"""
        import re as _re
        ...
        m = _re.search(r"(\d+)m(?!s)", s)
        if m:
            total += int(m.group(1)) * 60
        m = _re.search(r"(\d+(?:\.\d+)?)s(?!.*ms)", s)
        if m:
            total += _math.ceil(float(m.group(1)))
        m = _re.search(r"(\d+(?:\.\d+)?)ms", s)
        if m:
            total += _math.ceil(float(m.group(1)) / 1000)
        return total if total > 0 else None

    async def set_ag_token_status(self, token_id: str, status: str) -> bool:
        async with self._pool.acquire() as conn:
            res = await conn.execute(
                "UPDATE antigravity_tokens SET status=$1 WHERE id=$2", status, token_id,
            )
        return res == "UPDATE 1"

    # ══════════════════════════════════════════════════════════════════════
    # Orchids (Orchids AI Coding Agent) Token Pool
    # ══════════════════════════════════════════════════════════════════════

    _orch_rr_index = 0

    def _row_to_orchids_token(self, r) -> Dict:
        return {
            "id": r["id"],
            "email": r["email"],
            "client_cookie": r["client_cookie"],
    ...
    async def add_orchids_token(self, data: Dict) -> Dict:
        """添加 Orchids 凭证（__client cookie）。"""
        email = data.get("email", "")
        ...
        if email:
            async with self._pool.acquire() as conn:
                existing = await conn.fetchrow(
                    "SELECT id FROM orchids_tokens WHERE email = $1", email
                )
            if existing:
                tid = existing["id"]
                async with self._pool.acquire() as conn:
                    await conn.execute(
                        """UPDATE orchids_tokens SET
                           client_cookie=$1, session_id=$2, user_id=$3,
                           status='active', disabled_reason='',
                           error_count=0, cooldown_until=NULL
                           WHERE id=$4""",
                        client_cookie, data.get("session_id", ""),
                        data.get("user_id", ""), tid,
                    )
                logger.info(f"Orchids token updated: id={tid} email={email}")
                return {"id": tid, "email": email, "status": "active", "updated": True}

        tid = secrets.token_hex(8)
        async with self._pool.acquire() as conn:
            await conn.execute(
                """INSERT INTO orchids_tokens
                   (id, email, client_cookie, session_id, user_id, status, added_at)
                   VALUES ($1,$2,$3,$4,$5,'active',$6)""",
                tid, email, client_cookie,
                data.get("session_id", ""), data.get("user_id", ""), now,
            )
        logger.info(f"Orchids token added: id={tid} email={email}")
        return {"id": tid, "email": email, "status": "active"}

    async def list_orchids_tokens(self):
        """列出所有 Orchids 凭证（脱敏）。"""
        async with self._pool.acquire() as conn:
            rows = await conn.fetch("SELECT * FROM orchids_tokens ORDER BY added_at")
        result = []
        for r in rows:
            t = self._row_to_orchids_token(r)
            if t["client_cookie"]:
                t["client_cookie"] = t["client_cookie"][:20] + "..."
            result.append(t)
        return result

    async def remove_orchids_token(self, token_id: str) -> bool:
        async with self._pool.acquire() as conn:
            res = await conn.execute("DELETE FROM orchids_tokens WHERE id = $1", token_id)
        return res == "DELETE 1"

    async def get_orchids_token_full(self, token_id: str) -> Optional[Dict]:
        async with self._pool.acquire() as conn:
            r = await conn.fetchrow("SELECT * FROM orchids_tokens WHERE id = $1", token_id)
        if not r:
            return None
        return self._row_to_orchids_token(r)

    async def get_next_orchids_token(self) -> Optional[Dict]:
        """轮询获取下一个可用的 Orchids 凭证。"""
        now_utc = datetime.now(timezone.utc)
        async with self._pool.acquire() as conn:
            rows = await conn.fetch(
                """SELECT * FROM orchids_tokens
                   WHERE status = 'active'
                     AND (cooldown_until IS NULL OR cooldown_until < $1)
                   ORDER BY added_at""",
                now_utc,
        ...
        n = len(rows)
        idx = self._orch_rr_index % n
        self._orch_rr_index = (idx + 1) % n
        return self._row_to_orchids_token(rows[idx])

    async def mark_orchids_token_used(self, token_id: str):
        now = datetime.now(timezone.utc)
        async with self._pool.acquire() as conn:
            await conn.execute(
                "UPDATE orchids_tokens SET last_used=$1, use_count=use_count+1 WHERE id=$2",
                now, token_id,
            )

    async def update_orchids_token_info(self, token_id: str, session_id: str = "",
                                         user_id: str = "", email: str = ""):
        """更新 Orchids 凭证的 session/user 信息。"""
        sets, vals = [], []
        i = 1
        if session_id:
            sets.append(f"session_id = ${i}"); vals.append(session_id); i += 1
        if user_id:
            sets.append(f"user_id = ${i}"); vals.append(user_id); i += 1
        if email:
            sets.append(f"email = ${i}"); vals.append(email); i += 1
        if not sets:
            return
        vals.append(token_id)
        async with self._pool.acquire() as conn:
            await conn.execute(
                f"UPDATE orchids_tokens SET {', '.join(sets)} WHERE id = ${i}", *vals,
            )

    async def disable_orchids_token(self, token_id: str, reason: str = ""):
        async with self._pool.acquire() as conn:
            await conn.execute(
                "UPDATE orchids_tokens SET status='disabled', disabled_reason=$1 WHERE id=$2",
                reason, token_id,
            )
        logger.warning(f"Orchids token disabled: {token_id} reason={reason}")

    async def cooldown_orchids_token(self, token_id: str, seconds: int = 60):
        until = datetime.now(timezone.utc) + timedelta(seconds=seconds)
        async with self._pool.acquire() as conn:
            await conn.execute(
                "UPDATE orchids_tokens SET cooldown_until=$1, error_count=error_count+1 WHERE id=$2",
                until, token_id,
            )
        logger.info(f"Orchids token cooldown: {token_id} for {seconds}s")

    async def set_orchids_token_status(self, token_id: str, status: str) -> bool:
        async with self._pool.acquire() as conn:
            res = await conn.execute(
                "UPDATE orchids_tokens SET status=$1 WHERE id=$2", status, token_id,
            )
        return res == "UPDATE 1"
